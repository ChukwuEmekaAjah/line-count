import os
def read_directory(directory, files=[], exclude=[]):
...
def count_lines(file, flags = {}):
	...
	try:
		...
		if flags and bool(flags.get('no_comments')) is True:
			...
			print("number of lines in file ", file, ' is ', len(file_lines))
		...
		if flags and bool(flags.get('no_docs')) is True:
			...
			print("number of lines after removing docs in", file, "is", len(file_lines))
...
def remove_docs(file_lines):
	...
	return file_without_docs if len(file_without_docs) > 0 else file_lines
